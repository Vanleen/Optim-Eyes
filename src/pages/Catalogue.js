    ...
    useEffect(() => {
        ...
        fetchProducts().then((data) => {
          ...
          const filtered = data.filter((p) => 
            ...
            (p.subcategory && normalizeText(p.subcategory) === normalizeText(category))
          );
                ...
                <div className="flex justify-between items-center mb-8">
                    <div className="flex space-x-4">
                        {filtersByCategory[category]?.map((f) => (
                            <button key={f} className={`text-lg font-medium px-4 py-2 ${filter === f ? "underline text-[#ffaf50]" : "text-black"}`} onClick={() => handleFilterChange(f)}>
                                {f}
                            </button>
                        ))}
                    </div>
                    ...
                    <div className="relative">
                        <button className="text-lg font-medium flex items-center space-x-2 text-black" onClick={() => setShowSortMenu(!showSortMenu)}>
                        ...
                        {showSortMenu && (
                            <div className="absolute right-0 mt-2 bg-white shadow-md rounded-md w-48">
                                {sortingOptions.map((option) => (
                                    <button key={option} className="block w-full text-left px-4 py-2 hover:bg-gray-100" onClick={() => handleSortChange(option)}>
                                        {option}
                                    </button>
                                ))}
                            </div>
                    ...
                    {filteredProducts.length > 0 ? (
                        filteredProducts.map((product) => (
                            <div key={product.id} className="bg-white rounded-lg shadow-md overflow-hidden relative">
                                ...
                                <button className={`absolute bottom-3 right-3 text-xl ${likedProducts.includes(product.id) ? "text-red-500" : "text-gray-400"}`} onClick={() => toggleLike(product.id)}>
                                    {likedProducts.includes(product.id) ? <FiHeart className="fill-current text-red-500" /> : <FiHeart />}
                                </button>
